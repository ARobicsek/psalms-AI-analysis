...
- **Part 2 (user-approved)**: findings + `scripts/evaluate_novelty_ab.py` (blind full-commentary judge, DOCX fallback for OLD arm) → **main**; **R1–R6 implemented on branch `claude/sweet-fermat-pje3t7` only** + `scripts/run_novelty_ab.py` (re-run Pss 58/59/60 via the unchanged production harness into `output/ab_novelty/`, macro held constant, echoes file reused) + `docs/plans/NOVELTY_AB_TEST_RUNBOOK.md`. Run locally: `python scripts/run_novelty_ab.py 58 59 60` (~$17) then `python scripts/evaluate_novelty_ab.py 58 59 60` (~$3).
