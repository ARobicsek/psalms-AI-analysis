...
# synthesis-discovery sidecar (STEP 3.5), which is still very much alive.
...
def run_enhanced_pipeline(
    ...
    # See run_enhanced_pipeline.py: a now-deleted skip_insights defaulted to True
    # since Session 280, so leaving it here made every plain run load the old
    # stats file and duplicate its appended research-request lists.
    ...
    # import at the top of this file). The trimming stays: converse_with_editor.py
    # prefers psalm_NNN_research_trimmed.md over research_v2.md when it exists.
    ...
    if smoke_test:
        ...
        # Runs by default; skipped with --skip-synthesis-discovery. Overwrites
        # any prior observations file on every run.
        ...
        try:
            result = master_editor.write_commentary(
                ...
                micro_file=micro_file,
                research_file=research_file,
                # Session 374: always None. The {curated_insights} slot in the
                # writer prompt is deliberately left in place and renders the
                # constant "[No curated insights provided]" -- byte-identical to
...
if __name__ == "__main__":
    ...
    # accepted so existing invocations keep working; --skip/--exclude are now the
    # only behaviour, so they are silent no-ops. --include-insights WARNS below.
