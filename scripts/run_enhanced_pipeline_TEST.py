"""
Enhanced Pipeline Runner - PROMPT OVERHAUL TEST VERSION (V3 Prompts)

This is a COPY of run_enhanced_pipeline.py with THREE changes:
1. Imports MasterEditorV3 instead of MasterEditorV2
2. All output file names get a _TEST suffix
3. Pipeline output directory uses _test suffix by default

Usage:
    python scripts/run_enhanced_pipeline_TEST.py PSALM_NUMBER [options]

Example (reuse existing macro/micro/insights, only re-run writer):
    python scripts/run_enhanced_pipeline_TEST.py 100 --skip-macro --skip-micro --skip-insights
"""
...
def run_enhanced_pipeline(
    ...
    logger = get_logger("enhanced_pipeline_v3_test")
    logger.info(f"=" * 80)
    logger.info(f"ENHANCED PIPELINE (V3 PROMPT TEST) - Psalm {psalm_number}")
    if smoke_test:
    ...
    summary_json_file = output_path / f"psalm_{psalm_number:03d}_pipeline_stats_TEST.json"
    
    # === CHANGE 2: Read EXISTING source data from the ORIGINAL output directory ===
    # When skipping steps, we read from the ORIGINAL files (no _TEST suffix)
    # Only WRITER outputs get _TEST suffix
    source_output_path = Path(output_dir.replace("_test", ""))  # If user specifies _test dir, fall back to original
    if not source_output_path.exists():
        source_output_path = output_path  # Fall back to same dir
    ...
    # File paths — SOURCE files (read from original output, no _TEST)
    ...
    # === CHANGE 3: Writer outputs get _TEST suffix ===
    ...
    edited_assessment_file = output_path / f"psalm_{psalm_number:03d}_assessment_TEST.md"
    ...
    edited_assessment_college_file = output_path / f"psalm_{psalm_number:03d}_assessment_college_TEST.md"
    docx_output_college_file = output_path / f"psalm_{psalm_number:03d}_commentary_college_TEST.docx"
    docx_output_combined_file = output_path / f"psalm_{psalm_number:03d}_commentary_combined_TEST.docx"
    ...
    # =====================================================================
    # STEP 4: Master Writer V3 (Drafting & Editing)
    # =====================================================================
    if smoke_test:
        logger.info("\n[STEP 4] SMOKE TEST: Master Writer V3...")
        with open(edited_intro_file, 'w', encoding='utf-8') as f: f.write("# Smoke Test Intro (V3)")
        with open(edited_verses_file, 'w', encoding='utf-8') as f: f.write("# Smoke Test Verses (V3)")
        tracker.track_step_output("master_editor", "Smoke test output (V3)")
        
    elif not skip_writer:
        logger.info(f"\n[STEP 4] Running Master WRITER V3 ({master_editor_model})...")
        print(f"\n{'='*80}")
        print(f"STEP 4: Master Writer V3 ({master_editor_model})")
        print(f"{'='*80}\n")

        # === CHANGE 1 in effect: MasterEditor is now MasterEditorV3 ===
        ...
        try:
            ...
            # Save outputs (with _TEST suffix)
            ...
            if result.get('reader_questions'):
                refined_q_file = output_path / f"psalm_{psalm_number:03d}_reader_questions_refined_TEST.json"
                ...
                if questions:
                    with open(refined_q_file, 'w', encoding='utf-8') as f:
                        json.dump({
                            ...
                            'source': 'master_writer_v3_refined'
                        }, f, ensure_ascii=False, indent=2)
                    logger.info(f"Extracted {len(questions)} refined reader questions (V3)")
            ...
            logger.info(f"Master Writer V3 complete for Psalm {psalm_number}")
            print(f"  Introduction: {edited_intro_file}")
            print(f"  Verses: {edited_verses_file}\n")

        except openai.RateLimitError as e:
            logger.error(f"PIPELINE HALTED: OpenAI API quota exceeded during Master Writer V3 step. {e}")
            print(f"\nPIPELINE HALTED: OpenAI API quota exceeded. Please check your plan and billing details.")
            sys.exit(1)
        except Exception as e:
            logger.error(f"Master Writer V3 failed: {e}", exc_info=True)
            sys.exit(1)
            
        time.sleep(delay_between_steps)
    else:
        logger.info("[STEP 4] Skipping Master Writer V3")
        if not edited_intro_file.exists():
            logger.error("FATAL: Missing edited files")
            sys.exit(1)

    # =====================================================================
    # STEP 4b: College Writer V3
    # =====================================================================
    if not skip_college and not smoke_test:
        logger.info(f"\n[STEP 4b] Running College WRITER V3 ({master_editor_model})...")
        print(f"\n{'='*80}")
        print(f"STEP 4b: College Writer V3 ({master_editor_model})")
        print(f"{'='*80}\n")
        ...
        try:
            ...
            if 'input_char_count' in result:
                logger.info(f"College Writer V3 usage: {result['input_char_count']} chars input, {result.get('input_token_count', 0)} tokens")
            ...
            if result.get('reader_questions'):
                refined_questions_college_file = output_path / f"psalm_{psalm_number:03d}_reader_questions_college_refined_TEST.json"
                questions_text = result['reader_questions']
                questions = []
                for line in questions_text.strip().split('\n'):
                    line = line.strip()
                ...
                if questions:
                    with open(refined_questions_college_file, 'w', encoding='utf-8') as f:
                        json.dump({
                            'psalm_number': psalm_number,
                            'curated_questions': questions,
                            'source': 'college_writer_v3_refined'
                        }, f, ensure_ascii=False, indent=2)
                    logger.info(f"Extracted {len(questions)} college refined reader questions (V3)")

            logger.info(f"College Writer V3 complete for Psalm {psalm_number}")
            print(f"  College Introduction: {edited_intro_college_file}")
            print(f"  College Verses: {edited_verses_college_file}\n")

        except openai.RateLimitError as e:
            logger.error(f"PIPELINE HALTED: OpenAI API quota exceeded during College Writer V3 step. {e}")
            print(f"\nPIPELINE HALTED: OpenAI API quota exceeded. Please check your plan and billing details.")
            sys.exit(1)
        except Exception as e:
            logger.error(f"College Writer V3 failed: {e}", exc_info=True)

        time.sleep(delay_between_steps)

    # --- Save stats to disk before print-ready step ---
    tracker.mark_pipeline_complete()
    tracker.save_json(str(output_path))

    # =====================================================================
    # STEP 5: Print-Ready (SKIPPED FOR V3 TEST — uses _TEST files)
    # =====================================================================
    if not skip_print_ready:
        logger.info("[STEP 5] Print-Ready Formatting (V3 TEST)...")
        print(f"\n{'='*80}")
        print(f"STEP 5: Print-Ready Formatting (V3 TEST)")
        print(f"{'='*80}\n")
        print_ready_file = output_path / f"psalm_{psalm_number:03d}_print_ready_TEST.md"
    ...
    if not skip_word_doc:
        logger.info("[STEP 6] Word Doc Generation (V3 TEST)...")
        print(f"\n{'='*80}")
        print(f"STEP 6: Word Document Generation (.docx) (V3 TEST)")
        print(f"{'='*80}\n")
        from src.utils.document_generator import DocumentGenerator
        
        try:
            refined_q = output_path / f"psalm_{psalm_number:03d}_reader_questions_refined_TEST.json"
    ...
    if not skip_word_doc and not smoke_test and edited_intro_college_file.exists():
        logger.info("[STEP 6b] College Word Doc (V3 TEST)...")
        print(f"\n{'='*80}")
    ...
    if not skip_combined_doc and not smoke_test:
        if skip_college and not (edited_intro_college_file.exists() and edited_verses_college_file.exists()):
            logger.info(f"[STEP 6c] Skipping combined .docx generation")
        else:
            logger.info("\n[STEP 6c] Creating combined .docx document (main + college) (V3 TEST)...")
            print(f"\n{'='*80}")
            print(f"STEP 6c: Generating Combined Word Document (.docx) (V3 TEST)")
            print(f"{'='*80}\n")
            ...
            if all(f.exists() for f in required_files):
                try:
                    refined_questions_file = output_path / f"psalm_{psalm_number:03d}_reader_questions_refined_TEST.json"
                    refined_questions_college_file = output_path / f"psalm_{psalm_number:03d}_reader_questions_college_refined_TEST.json"
                    ...
                    logger.info(f"Successfully generated combined Word document for Psalm {psalm_number} (V3 TEST).")
    ...
    logger.info(f"\n{'=' * 80}")
    logger.info(f"ENHANCED PIPELINE (V3 PROMPT TEST) COMPLETE - Psalm {psalm_number}")
    logger.info(f"{'=' * 80}\n")

    print(f"\n{'='*80}")
    print(f"PIPELINE COMPLETE (V3 PROMPT TEST) - Psalm {psalm_number}")
    print(f"{'='*80}")
    print(cost_tracker.get_summary())

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Enhanced Pipeline V3 PROMPT TEST")
    ...
    # Set output directory — SAME as original (we write to the same folder, with _TEST suffix on filenames)
    ...
    print(f"\n{'='*80}")
    print(f"ENHANCED COMMENTARY PIPELINE (V3 PROMPT TEST) - Psalm {args.psalm_number}")
    print(f"{'='*80}\n")
    print(f"Output Directory: {args.output_dir}")
    print(f"Database: {args.db_path}")
    print(f"Rate Limit Delay: {args.delay} seconds")
    print(f"Master Writer Model: {args.master_editor_model}")
    print(f"NOTE: All writer outputs suffixed with _TEST")
    print()
