...
def run_enhanced_pipeline(
    ...
    # SI-specific filenames
    edited_intro_file = output_path / f"psalm_{psalm_number:03d}_edited_intro_SI.md"
    edited_verses_file = output_path / f"psalm_{psalm_number:03d}_edited_verses_SI.md"
    edited_assessment_file = output_path / f"psalm_{psalm_number:03d}_assessment_SI.md"
    docx_output_file = output_path / f"psalm_{psalm_number:03d}_commentary_SI.docx"
    
    # College edition file paths
    edited_intro_college_file = output_path / f"psalm_{psalm_number:03d}_edited_intro_college_SI.md"
    edited_verses_college_file = output_path / f"psalm_{psalm_number:03d}_edited_verses_college_SI.md"
    edited_assessment_college_file = output_path / f"psalm_{psalm_number:03d}_assessment_college_SI.md"
    ...
    if smoke_test:
        ...
        # Still need to get model name for tracking
        EditorClass = MasterEditorOld if master_editor_old else MasterEditorSI
        master_editor = EditorClass(
            main_model=master_editor_model,
            cost_tracker=cost_tracker
        )
...
Examples:
  ...
  python scripts/run_enhanced_pipeline.py 2 --skip-master-edit
    ...
    parser.add_argument('--special-instruction', type=str, default=None,
                       help='Path to a text file containing special instructions for the Master Editor.')
