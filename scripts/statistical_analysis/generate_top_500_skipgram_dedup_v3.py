"""
Generate Top 500 Detailed Connections with Skipgram-Aware Deduplication V3

Changes from V2:
1. Uses V3 scoring with verse-level details
2. Enhanced output format includes:
   - Verse numbers for all matches
   - Position information for contiguous phrases
   - Full span Hebrew text for skipgrams
   - Position ranges for skipgrams
   - Verse details for roots

Creates comprehensive JSON showing top 500 connections with complete verse-level information.
"""
...
def main():
    """Generate detailed top 500 connections with skipgram-aware deduplicated scoring V3."""
    
    base_dir = Path(__file__).parent.parent.parent
    
    # Load skipgram-aware deduplicated scores V3
    logger.info("Loading skipgram-aware deduplicated scores V3...")
    input_file = base_dir / "data/analysis_results/enhanced_scores_skipgram_dedup_v3.json"
    
    if not input_file.exists():
        logger.error(f"Input file not found: {input_file}")
        logger.error("Please run enhanced_scorer_skipgram_dedup_v3.py first.")
        return
    ...
    logger.info("Adding rank and preparing V3 output with verse-level details...")
    top_500_detailed = []
    ...
    for i, entry in enumerate(top_500, 1):
        detailed_entry = {
            ...
            # ENHANCED V3: DETAILED MATCHES WITH VERSE-LEVEL INFORMATION
    ...
    # Save output
    output_path = base_dir / "data/analysis_results/top_500_connections_skipgram_dedup_v3.json"
    ...
    # V3 specific: Check verse details coverage
    phrases_with_verses = sum(
        1 for e in top_500_detailed
        for p in e['deduplicated_contiguous_phrases']
        if p.get('matches_from_a') or p.get('matches_from_b')
    )
    skipgrams_with_verses = sum(
        1 for e in top_500_detailed
        for s in e['deduplicated_skipgrams']
        if s.get('matches_from_a') or s.get('matches_from_b')
    )
    roots_with_verses = sum(
        1 for e in top_500_detailed
        for r in e['deduplicated_roots']
        if r.get('matches_from_a') or r.get('matches_from_b')
    )
    
    logger.info(f"\nV3 Verse-level details coverage:")
    logger.info(f"  Contiguous phrases with verse details: {phrases_with_verses:,}")
    logger.info(f"  Skipgrams with verse details: {skipgrams_with_verses:,}")
    logger.info(f"  Roots with verse details: {roots_with_verses:,}")
    
    logger.info("\n✓ Complete! V3 output includes verse-level details for all matches.")
