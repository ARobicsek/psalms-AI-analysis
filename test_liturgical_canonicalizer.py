"""
Test script for liturgical canonicalization pipeline.
Processes only the first 3 prayers to validate the system.
"""
...
def create_user_prompt(prayer_data):
    ...
    input_data = {
        ...
        "hebrew_text": prayer_data.get("hebrew_text", "")[:500]  # Truncate for test
    }
...
def main():
    print("=" * 70)
    print("TESTING LITURGICAL CANONICALIZATION PIPELINE")
    print("=" * 70)
    ...
    cursor.execute("""
        SELECT prayer_id, source_text, nusach, service, section, prayer_name, hebrew_text
        FROM prayers
        ORDER BY prayer_id
        LIMIT 3
    """)
    ...
    print(f"\nProcessing {len(prayers)} test prayers...\n")
    ...
    for i, prayer in enumerate(prayers, 1):
        ...
        print(f"[{i}/3] Prayer ID {prayer_id}: {prayer_name}")
