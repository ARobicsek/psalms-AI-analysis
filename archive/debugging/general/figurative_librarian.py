...
Queries the existing figurative language database (Pentateuch + Psalms + Proverbs + Isaiah).
...
- Contains figurative instances from Pentateuch, Psalms, Proverbs, and Isaiah
...
class FigurativeRequest:
    ...
    # Priority ranking for search terms (NEW for priority-based filtering)
    priority_ranking: Optional[Dict[str, int]] = None  # {"phrase_X": 1, "word_Y": 2, "word_Z": 3}
    # Priority 1 = highest priority, matched first
...
class FigurativeLibrarian:
    ...
    def get_available_books(self) -> List[str]:
        ...
        Example:
            >>> librarian = FigurativeLibrarian()
            >>> books = librarian.get_available_books()
            >>> print(books)
            ['Deuteronomy', 'Exodus', 'Genesis', 'Isaiah', 'Leviticus', 'Numbers', 'Proverbs', 'Psalms']
        """
