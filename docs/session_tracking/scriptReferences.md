...
| `run_scripture_verifier.py` | `scripts` | Standalone scripture citation verifier. Checks quoted Hebrew against `tanakh.db`. Supports batch psalms, `--fix` for copy-editor fix pass, `--haiku-filter` for false-positive filtering (~$0.002/psalm). | [file](file:///c:/Users/ariro/OneDrive/Documents/Psalms/scripts/run_scripture_verifier.py) |
...
| `scripture_verifier.py` | `src.utils` | Scripture citation verifier. Extracts Hebrew quotations via 4 patterns: A/B (Hebrew before parenthetical citation), C (Hebrew inside parenthetical), D (inline forward references like `2 Samuel 7:29 —`). Normalizes text (cantillation, meteg, divine names incl. full Tetragrammaton, word-level consonantal matching). Ellipsis-fragment splitting for `...`-separated quotes. Optional Haiku false-positive filter (`filter_false_positives()`, ~$0.002/psalm). Pipeline Step 5a½. | [file](file:///c:/Users/ariro/OneDrive/Documents/Psalms/src/utils/scripture_verifier.py) |
