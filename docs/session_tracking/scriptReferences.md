...
| `run_beta_reader.py` | `scripts` | **NEW (Session 360)**: Standalone beta-reader runner. Simulated target-audience read of the finished guide (`copy_edited.md`, fallback `print_ready.md`) producing a reader-EXPERIENCE report — engagement curve, quoted AHA/WIT/FELT moments, affective-landing check, sag points, confusion, 5 parsed scores. Measurement-only (no edits; feeds no revision pass). Batch psalms, `--input-file`, `--model` (default `claude-sonnet-4-6`, ~$0.08/psalm). Also runs as pipeline STEP 5d by default (`--skip-beta-reader`). Output: `psalm_NNN_beta_read.md`. | [file](file:///c:/Users/ariro/OneDrive/Documents/Psalms/scripts/run_beta_reader.py) |
