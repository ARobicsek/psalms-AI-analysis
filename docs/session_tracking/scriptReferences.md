...
| `scripture_verifier.py` | `src.utils` | Scripture citation verifier. Extracts Hebrew quotations with parenthetical citations, normalizes text (cantillation, divine names, word-level consonantal matching), checks against `tanakh.db`. Optional Haiku false-positive filter (`filter_false_positives()`, ~$0.002/psalm). Pipeline Step 5a½. | [file](file:///c:/Users/ariro/OneDrive/Documents/Psalms/src/utils/scripture_verifier.py) |
