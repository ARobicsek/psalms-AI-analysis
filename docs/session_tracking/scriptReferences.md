...
| `run_novelty_ab.py` | `scripts` | **A/B HARNESS (Session 358, THIS BRANCH ONLY)**: re-runs psalms through the production `run_enhanced_pipeline.py` (identical harness/defaults) but with the branch's R1-R6 changes, into isolated `output/ab_novelty/psalm_N` dirs. Holds macro constant (copies baseline `psalm_0NN_macro.json` + `--skip-macro`) and reuses the existing literary-echoes file (`--skip-lit-echoes`) so arm differences isolate the R changes. Refuses to run off-branch (`--allow-any-branch` to override); `--dry-run`, `--fresh-macro`, `--with-lit-echoes`, `--delay`. ~$5.5-6/psalm. See `docs/plans/NOVELTY_AB_TEST_RUNBOOK.md`. | [file](file:///c:/Users/ariro/OneDrive/Documents/Psalms/scripts/run_novelty_ab.py) |
| `evaluate_novelty_ab.py` | `scripts` | **JUDGE (Session 358)**: blind LLM judge + deterministic diff for FULL-COMMENTARY A/B comparisons (built for the R1-R6 novelty A/B, but generic old-vs-new). Per psalm: deterministic metrics (sizes, hedge-marker counts, copy-editor change-log stats, citation issues, cost.json) + a blind Opus judge that inventories each commentary's distinct insights (NOV/GRD/AHA 1-5, INTERNAL vs BRIDGING, GOLD/GOOD/FILLER), audits overreach/false-profundity/lost-hedges, spot-checks verse coverage, and names a winner (unblinded in the saved report + cross-psalm summary table). OLD arm resolves from `output/psalm_N` or falls back to the published DOCX in `Documents/Psalm study guide/`; NEW arm from `output/ab_novelty/psalm_N` (templates overridable via `--old-dir`/`--new-dir`). `--skip-judge` for $0 metrics-only. ~$0.8-1.5/psalm judged. | [file](file:///c:/Users/ariro/OneDrive/Documents/Psalms/scripts/evaluate_novelty_ab.py) |
