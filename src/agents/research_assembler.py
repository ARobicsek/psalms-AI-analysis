...
class ResearchBundle:
    ...
    def to_markdown(self) -> str:
        ...
        md += f"- **Literary echoes included**: {'Yes' if summary['literary_echoes_included'] else 'No'}\n"
...
class ResearchAssembler:
    ...
    def __init__(self, use_llm_summaries: bool = True, cost_tracker=None, use_figurative_curator: bool = True):
        ...
        if use_figurative_curator:
            try:
                self.figurative_curator = FigurativeCurator(verbose=False, cost_tracker=cost_tracker)
                if self.logger:
        ...
        # Literary echoes directory path (resolved relative to project root)
        self._literary_echoes_dir = Path(__file__).parent.parent.parent / 'data' / 'literary_echoes'
    ...
    def _load_literary_echoes(self, psalm_chapter: int) -> Optional[str]:
        """
        Load literary echoes content from file if it exists.

        These files contain cross-cultural literary and poetic comparisons
        generated by a separate Gemini Deep Research run focused specifically
        on world literature connections to the psalm.

        Args:
            psalm_chapter: Psalm number to look up

        Returns:
            Literary echoes content as string, or None if not found
        """
        filename = f"psalm_{psalm_chapter:03d}_literary_echoes.txt"
        filepath = self._literary_echoes_dir / filename

        if filepath.exists():
            try:
                content = filepath.read_text(encoding='utf-8').strip()
                if content:
                    self.logger.info(f"Loaded literary echoes for Psalm {psalm_chapter}: {len(content)} chars")
                    return content
            except Exception as e:
                self.logger.warning(f"Failed to load literary echoes for Psalm {psalm_chapter}: {e}")
